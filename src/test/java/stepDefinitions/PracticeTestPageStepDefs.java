...
public void downYazisindakiIkiBasamakliSayilarinHepsiniToplayin() {
    List<WebElement> down32=practiceTestPage.down32; //pageobje
    üzerinden cektim ve listime attım buradaki iki basamakli
    sayilari toplamamlazım, suan burada 50 tane down32 var
    for (WebElement w:down32) {//down32dekileri w ye alıyorum
        w.getText().replaceAll("^[0-9]",""); //her bir
         webelementindeki down32 yi sırayla cektim ve w nin
          icine koyuorum, getText ile down32 olarak alıyorum sadece
          32 yi almak icin
         // regex yani replaceAll ile rakam
         olmayanları at diyorum (\\D) kullanarakta yapabilirdim.
 */
